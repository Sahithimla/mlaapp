...
public class LoginController {
	...
	@PostMapping("/login")
	public ResponseEntity<Dictionary> login(@RequestBody LoginDTO loginDTO)
			throws ValidationFailedException, MLAServiceException {
		logger.debug("LoginController :: login -> START");
		Dictionary<String, String> userResp = this.loginService.login(loginDTO);
		logger.debug("LoginController :: login -> END");
		return new ResponseEntity<Dictionary>(userResp, HttpStatus.OK);
	}
